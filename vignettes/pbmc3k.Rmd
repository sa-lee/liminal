...
pcs <- tibble::as_tibble(reducedDim(pbmc3k, "pca"))
...
```{r}
limn_xy(limn_tbl, x= tsne_x, y = tsne_y, color = cluster)
```
...
```{r}
limn_tour_xylink(limn_tbl, PC1:PC10, 
                 x=tsne_x, y=tsne_y, 
           colors = c(CD79A, MS4A1, CD8A, CD8B, LYZ), transformer)
```
